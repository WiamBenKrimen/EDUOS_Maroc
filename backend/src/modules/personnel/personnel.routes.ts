...
import type { PersonnelFonction } from '../../types/auth.js'
...
export const personnelRoutes: FastifyPluginAsync = async app => {
  const guards = [app.authenticate, authorize('personnel:manage')]

  app.get('/', { preHandler: guards }, async request => {
    return query<PersonnelRow>(
      `SELECT id, nom, prenom, email, telephone, personnel_fonction, statut, created_at
         FROM users
