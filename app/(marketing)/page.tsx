...
function Navbar({ scrolled, onOpenCandidature }: { scrolled: boolean; onOpenCandidature: () => void }) {
    ...
    <header className={`navbar${scrolled ? ' scrolled' : ''}`}>
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '0 28px', height: 72, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <nav className="hide-mob" style={{ display: 'flex', gap: 2 }}>
          {NAV_LINKS.map(({ label, target, href }) => (
            href ? (
              <Link key={label} href={href} className="nav-link" style={{ textDecoration: 'none' }}>{label}</Link>
            ) : (
        ...
        <div className="hide-mob" style={{ display: 'flex', gap: 10 }}>
          ...
          <button className="btn-navy" onClick={onOpenCandidature}>Candidature simplifiée</button>
        </div>
      ...
      {open && (
        <div style={{ background: '#fff', borderTop: '1px solid var(--border)', padding: '16px 28px 24px' }}>
          {NAV_LINKS.map(({ label, target, href }) => (
            href ? (
              <Link key={label} href={href} className="nav-link" style={{ display: 'block', marginBottom: 6, textDecoration: 'none' }} onClick={() => setOpen(false)}>{label}</Link>
            ) : (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginTop: 14 }}>
            ...
            <button className="btn-navy" onClick={() => { onOpenCandidature(); setOpen(false) }}>Candidature simplifiée</button>
          </div>
        </div>
      )}
    </header>
...
/* ── Hero ────────────────────────────────────────────────── */
function Hero({ onOpenCandidature }: { onOpenCandidature: () => void }) {
  return (
    <section id="accueil" style={{ paddingTop: 112, paddingBottom: 80, background: '#fff', overflow: 'hidden', position: 'relative' }}>
      ...
      <div style={{ maxWidth: 860, margin: '0 auto', padding: '0 28px', textAlign: 'center', position: 'relative', zIndex: 1 }}>
        ...
        <div className="anim-up d4" style={{ display: 'flex', justifyContent: 'center', gap: 12, flexWrap: 'wrap' }}>
          <button className="btn-navy" style={{ fontSize: '1rem', padding: '15px 32px', borderRadius: 10 }} onClick={onOpenCandidature}>Candidater mon centre</button>
          <Link href="/comment-ca-marche" className="btn-outline" style={{ fontSize: '1rem', padding: '15px 32px', borderRadius: 10, textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 8 }}>Comment ça marche</Link>
        </div>
...
/* ── CTASection ──────────────────────────────────────────── */
function CTASection({ onOpenCandidature }: { onOpenCandidature: () => void }) {
  const r = useReveal(0)
  return (
      ...
      <div style={{ maxWidth: 700, margin: '0 auto' }}>
        <div ref={r} className="reveal" style={{ textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: 12, flexWrap: 'wrap', marginBottom: 18 }}>
            <button className="btn-navy" style={{ fontSize: '1.02rem', padding: '14px 30px' }} onClick={onOpenCandidature}>Candidature simplifiée</button>
            <button className="btn-outline" style={{ fontSize: '1.02rem', padding: '14px 30px' }} onClick={onOpenCandidature}>Candidater mon centre</button>
          </div>
          <p style={{ fontSize: '.78rem', color: '#bbb', fontFamily: 'var(--font-body)' }}>Aucune carte bancaire · Déploiement offert en 24h · Support dédié</p>
        </div>
      </div>
    </section>
  )
...
function CandidatureModal({ onClose }: { onClose: () => void }) {
  ...
  const [form, setForm] = useState({
    ...
    email: '',
    ville: 'Casablanca',
    tailleEleves: '50 à 200 élèves',
    besoins: ['Paiements & Mensualités', 'Relances WhatsApp auto', 'Feuille de présence QR'],
    remarques: ''
  })
  ...
  const handleToggleBesoin = (b: string) => {
    setForm(prev => ({
      ...prev,
      besoins: prev.besoins.includes(b) ? prev.besoins.filter(x => x !== b) : [...prev.besoins, b]
    }))
  }
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 1200, background: 'rgba(9,24,46,.65)', backdropFilter: 'blur(5px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ width: '100%', maxWidth: 540, maxHeight: '90vh', overflowY: 'auto', background: '#fff', borderRadius: 16, border: '1px solid #E2E8F0', boxShadow: '0 16px 40px rgba(0,0,0,.2)', padding: 28 }}>
        {!submitted ? (
          <form onSubmit={handleSubmit}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 18, borderBottom: '1px solid #F1F5F9', paddingBottom: 14 }}>
              <div>
                ...
                <h2 style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: '1.25rem', color: '#1B3A6B', margin: '0 0 4px' }}>
                  Candidature Centre de Formation
                </h2>
                <p style={{ fontSize: '.8rem', color: '#64748B', margin: 0 }}>
                  Remplissez ce formulaire pour faire bénéficier votre centre de la plateforme EDUOS.
                </p>
              </div>
              <button type="button" onClick={onClose} style={{ background: '#F1F5F9', border: 'none', borderRadius: 8, padding: '6px 10px', cursor: 'pointer', fontWeight: 700, color: '#64748B' }}>✕</button>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div>
                ...
                <input
                  required
                  style={{ width: '100%', padding: '10px 14px', borderRadius: 9, border: '1px solid #CBD5E1', fontFamily: 'var(--font-body)', fontSize: '.85rem', outline: 'none', boxSizing: 'border-box' }}
                  placeholder="ex: Centre Atlas de Formation Continue"
                  value={form.centreNom}
                  onChange={e => setForm(prev => ({ ...prev, centreNom: e.target.value }))}
                />
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div>
                  ...
                  <input
                    required
                    style={{ width: '100%', padding: '10px 14px', borderRadius: 9, border: '1px solid #CBD5E1', fontFamily: 'var(--font-body)', fontSize: '.85rem', outline: 'none', boxSizing: 'border-box' }}
                    placeholder="ex: Dr. Rachid Bennani"
                    value={form.responsableNom}
                    onChange={e => setForm(prev => ({ ...prev, responsableNom: e.target.value }))}
                  />
                </div>
                <div>
                  <label style={{ display: 'block', fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: '.78rem', color: '#334155', marginBottom: 5 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div>
                  ...
                  <input
                    type="email"
                    style={{ width: '100%', padding: '10px 14px', borderRadius: 9, border: '1px solid #CBD5E1', fontFamily: 'var(--font-body)', fontSize: '.85rem', outline: 'none', boxSizing: 'border-box' }}
                    placeholder="direction@votrecentre.ma"
                    value={form.email}
                ...
                <div>
                  ...
                  <select
                    style={{ width: '100%', padding: '10px 14px', borderRadius: 9, border: '1px solid #CBD5E1', fontFamily: 'var(--font-body)', fontSize: '.85rem', outline: 'none', boxSizing: 'border-box', background: '#fff' }}
                    value={form.ville}
              ...
              <div>
                <label style={{ display: 'block', fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: '.78rem', color: '#334155', marginBottom: 5 }}>
                  Nombre d'apprenants estimé
                </label>
                <select
                  style={{ width: '100%', padding: '10px 14px', borderRadius: 9, border: '1px solid #CBD5E1', fontFamily: 'var(--font-body)', fontSize: '.85rem', outline: 'none', boxSizing: 'border-box', background: '#fff' }}
                  value={form.tailleEleves}
                  onChange={e => setForm(prev => ({ ...prev, tailleEleves: e.target.value }))}
                >
                  <option>Moins de 50 élèves</option>
                  <option>50 à 200 élèves</option>
                  <option>200 à 500 élèves</option>
                  <option>Plus de 500 élèves</option>
                </select>
              </div>

              <div>
                <label style={{ display: 'block', fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: '.78rem', color: '#334155', marginBottom: 6 }}>
                  Fonctionnalités prioritaires recherchées
                </label>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                  {[
                    ...
                    <label key={item} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: '.76rem', color: '#475569', cursor: 'pointer', background: '#F8FAFC', padding: '6px 10px', borderRadius: 8, border: '1px solid #E2E8F0' }}>
                      <input
                        type="checkbox"
                        checked={form.besoins.includes(item)}
                        onChange={() => handleToggleBesoin(item)}
                      />
                      {item}
                    </label>
                  ))}
              ...
              <div>
                ...
                <textarea
                  rows={2}
                  style={{ width: '100%', padding: '10px 14px', borderRadius: 9, border: '1px solid #CBD5E1', fontFamily: 'var(--font-body)', fontSize: '.85rem', outline: 'none', resize: 'vertical', boxSizing: 'border-box' }}
                  placeholder="Décrivez vos besoins ou questions pour notre équipe..."
          ...
          <div style={{ textAlign: 'center', padding: '16px 8px' }}>
            <div style={{ width: 56, height: 56, borderRadius: '50%', background: '#059669', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px', fontSize: '1.5rem', fontWeight: 800 }}>
              ✓
            </div>
            <span style={{ display: 'inline-block', padding: '4px 12px', borderRadius: 99, background: 'rgba(27,58,107,.09)', color: '#1B3A6B', fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: '.78rem', marginBottom: 10 }}>
              Référence : {refNumber}
            </span>
            <h2 style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: '1.4rem', color: '#1B3A6B', margin: '0 0 8px' }}>
              Candidature envoyée avec succès !
            </h2>
            <p style={{ fontSize: '.88rem', lineHeight: 1.6, marginBottom: 20, color: '#475569' }}>
              La candidature du centre <strong>{form.centreNom}</strong> pour <strong>{form.responsableNom}</strong> a été transmise à l'équipe EDUOS Maroc.
            </p>

            <div style={{ background: '#F8FAFC', borderRadius: 12, padding: '16px', border: '1px solid #E2E8F0', marginBottom: 24, textAlign: 'left' }}>
              <div style={{ fontFamily: "var(--font-display)", fontWeight: 700, fontSize: '.82rem', color: '#059669', marginBottom: 4 }}>
                💬 Confirmation WhatsApp sous 24h
              </div>
              <p style={{ fontSize: '.78rem', color: '#64748B', margin: 0, lineHeight: 1.5 }}>
                Un conseiller pédagogique EDUOS vous contactera directement sur votre numéro <strong>{form.telephone}</strong> pour valider l'éligibilité de votre centre et configurer votre espace d'essai offert.
              </p>
            </div>

            <button className="btn-navy" style={{ width: '100%', justifyContent: 'center', padding: '12px' }} onClick={onClose}>
...
function HowItWorksSection() {
  const steps = [
    {
      num: '01',
      title: 'Candidatez votre centre',
      desc: 'Remplissez le formulaire de candidature en moins de 3 minutes. Notre équipe vous contacte sous 24h pour valider votre dossier.',
      ...
      color: '#1B3A6B', bg: 'rgba(27,58,107,.08)',
    },
    {
      num: '02',
      title: 'Configuration en 24h',
      desc: 'Notre équipe configure votre espace EDUOS : centre, formations, groupes et utilisateurs. Vous recevez vos accès le lendemain.',
      icon: (<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 00.33 1.82l.06.06a2 2 0 010 2.83 2 2 0 01-2.83 0l-.06-.06a1.65 1.65 0 00-1.82-.33 1.65 1.65 0 00-1 1.51V21a2 2 0 01-4 0v-.09A1.65 1.65 0 009 19.4a1.65 1.65 0 00-1.82.33l-.06.06a2 2 0 01-2.83-2.83l.06-.06A1.65 1.65 0 004.68 15a1.65 1.65 0 00-1.51-1H3a2 2 0 010-4h.09A1.65 1.65 0 004.6 9a1.65 1.65 0 00-.33-1.82l-.06-.06a2 2 0 012.83-2.83l.06.06A1.65 1.65 0 009 4.68a1.65 1.65 0 001-1.51V3a2 2 0 014 0v.09a1.65 1.65 0 001 1.51 1.65 1.65 0 001.82-.33l.06-.06a2 2 0 012.83 2.83l-.06.06A1.65 1.65 0 0019.4 9a1.65 1.65 0 001.51 1H21a2 2 0 010 4h-.09a1.65 1.65 0 00-1.51 1z"/></svg>),
      color: '#C9922A', bg: 'rgba(201,146,42,.09)',
    },
    {
      num: '03',
      title: 'Formez vos équipes',
      desc: 'Une session de formation live de 2h pour vos administrateurs et formateurs. Guide vidéo et support WhatsApp inclus pour vos premiers jours.',
    ...
    {
      num: '04',
      title: 'Pilotez et développez',
      desc: 'Vos équipes gèrent inscriptions, présences et paiements depuis la plateforme. Vous suivez les indicateurs clés depuis votre tableau de bord.',
  ...
  return (
    <section id="comment-ca-marche" style={{ padding: '100px 28px', background: '#FAFBFC', borderTop: '1px solid #E4E8EE' }}>
      <div style={{ maxWidth: 1100, margin: '0 auto' }}>
        <div style={{ textAlign: 'center', marginBottom: 64 }}>
          <span className="label-pill-navy" style={{ marginBottom: 18, display: 'inline-flex' }}>Mise en route</span>
          <h2 style={{ fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: 'clamp(1.8rem, 3.2vw, 2.8rem)', lineHeight: 1.18, letterSpacing: '-.03em', color: 'var(--text)', marginBottom: 16 }}>
            Comment ça marche ?
          </h2>
          <p style={{ fontSize: '1.05rem', color: 'var(--muted)', maxWidth: 520, margin: '0 auto', lineHeight: 1.7 }}>
            De la candidature au pilotage complet — votre centre est opérationnel en moins de 48h.
          </p>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 24, position: 'relative' }}>
          {/* Connector line */}
          <div style={{ position: 'absolute', top: 36, left: '12.5%', right: '12.5%', height: 2, background: 'linear-gradient(to right, rgba(27,58,107,.12), rgba(201,146,42,.25), rgba(5,150,105,.2), rgba(124,58,237,.15))', zIndex: 0 }} />
          {steps.map((s, i) => (
            <div key={i} style={{ background: '#fff', borderRadius: 18, padding: '28px 24px 32px', border: '1px solid rgba(27,58,107,.07)', boxShadow: '0 4px 20px rgba(27,58,107,.06)', position: 'relative', zIndex: 1, textAlign: 'center', transition: 'transform .2s, box-shadow .2s' }}
              onMouseEnter={e => { (e.currentTarget as HTMLDivElement).style.transform = 'translateY(-4px)'; (e.currentTarget as HTMLDivElement).style.boxShadow = '0 12px 36px rgba(27,58,107,.12)' }}
              onMouseLeave={e => { (e.currentTarget as HTMLDivElement).style.transform = 'translateY(0)'; (e.currentTarget as HTMLDivElement).style.boxShadow = '0 4px 20px rgba(27,58,107,.06)' }}
            >
              {/* Step number badge */}
              <div style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: 56, height: 56, borderRadius: 16, background: s.bg, color: s.color, marginBottom: 20, position: 'relative' }}>
                <div style={{ width: 26, height: 26 }}>{s.icon}</div>
                <div style={{ position: 'absolute', top: -8, right: -8, width: 22, height: 22, borderRadius: '50%', background: s.color, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: '.58rem', color: '#fff', letterSpacing: '.03em' }}>{s.num}</div>
              </div>
              <h3 style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: '1rem', color: 'var(--text)', marginBottom: 12, lineHeight: 1.3 }}>{s.title}</h3>
              <p style={{ fontSize: '.84rem', color: 'var(--muted)', lineHeight: 1.65, margin: 0 }}>{s.desc}</p>
            </div>
          ))}
        </div>
        {/* CTA */}
        <div style={{ textAlign: 'center', marginTop: 56 }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '12px 20px', borderRadius: 12, background: 'rgba(5,150,105,.07)', border: '1px solid rgba(5,150,105,.15)', marginBottom: 28 }}>
            ...
            <span style={{ fontSize: '.82rem', fontWeight: 700, color: '#059669', fontFamily: 'var(--font-display)' }}>Déploiement en 24h · Support WhatsApp inclus · Sans engagement</span>
          </div>
        </div>
      </div>
    </section>
...
function PricingSection() {
  ...
  const plans = [
    {
      name: 'Starter',
      desc: 'Idéal pour un petit centre qui démarre',
      priceMonthly: 299,
      priceAnnual: 249,
      currency: 'DH/mois',
      color: '#1B3A6B',
      bg: 'rgba(27,58,107,.05)',
      ...
      disabled: ['Relances WhatsApp automatiques', 'Rapports pédagogiques avancés', 'API & intégrations'],
    },
    {
      name: 'Pro',
      desc: 'La solution complète pour les centres actifs',
      priceMonthly: 599,
      priceAnnual: 499,
      currency: 'DH/mois',
      color: '#C9922A',
      ...
      disabled: ['API & intégrations'],
    },
    {
      name: 'Entreprise',
      desc: 'Pour les réseaux de centres et grandes structures',
      priceMonthly: 0,
      priceAnnual: 0,
      currency: 'Sur devis',
      color: '#059669',
  ...
  return (
    <section id="tarifs" style={{ padding: '100px 28px', background: '#fff', borderTop: '1px solid #E4E8EE' }}>
      <div style={{ maxWidth: 1100, margin: '0 auto' }}>
        {/* Header */}
        <div style={{ textAlign: 'center', marginBottom: 52 }}>
          <span className="label-pill" style={{ marginBottom: 18, display: 'inline-flex', fontSize: '.75rem', letterSpacing: '.06em' }}>Tarifs transparents</span>
          <h2 style={{ fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: 'clamp(1.8rem, 3.2vw, 2.8rem)', lineHeight: 1.18, letterSpacing: '-.03em', color: 'var(--text)', marginBottom: 16 }}>
            Une offre adaptée à votre centre
          </h2>
          <p style={{ fontSize: '1.05rem', color: 'var(--muted)', maxWidth: 500, margin: '0 auto 32px', lineHeight: 1.7 }}>
            Choisissez le plan qui correspond à la taille de votre structure. Sans engagement, sans frais cachés.
          </p>
          {/* Annual toggle */}
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 12, padding: '6px', background: '#F2F5F9', borderRadius: 12, border: '1px solid #E4E8EE' }}>
            <button onClick={() => setAnnual(false)} style={{ padding: '8px 18px', borderRadius: 8, border: 'none', fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: '.82rem', cursor: 'pointer', background: !annual ? '#fff' : 'transparent', color: !annual ? 'var(--navy)' : '#64748b', boxShadow: !annual ? '0 1px 4px rgba(0,0,0,.08)' : 'none', transition: 'all .2s' }}>
              Mensuel
            </button>
            <button onClick={() => setAnnual(true)} style={{ padding: '8px 18px', borderRadius: 8, border: 'none', fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: '.82rem', cursor: 'pointer', background: annual ? '#fff' : 'transparent', color: annual ? 'var(--navy)' : '#64748b', boxShadow: annual ? '0 1px 4px rgba(0,0,0,.08)' : 'none', transition: 'all .2s', display: 'flex', alignItems: 'center', gap: 7 }}>
              Annuel
              <span style={{ fontSize: '.68rem', fontWeight: 800, color: '#fff', background: '#059669', borderRadius: 99, padding: '2px 7px' }}>-17%</span>
            </button>
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 22, alignItems: 'stretch' }}>
          {plans.map((plan, i) => {
            ...
            return (
              <div key={i} style={{
                borderRadius: 20,
                padding: isPro ? '36px 32px' : '32px 28px',
                border: `1.5px solid ${plan.border}`,
                background: isPro ? plan.bg : plan.bg,
                position: 'relative',
                boxShadow: isPro ? '0 24px 64px rgba(27,58,107,.28)' : '0 4px 20px rgba(27,58,107,.05)',
                transform: isPro ? 'scale(1.04)' : 'scale(1)',
                transition: 'transform .2s, box-shadow .2s',
              }}
                onMouseEnter={e => { if (!isPro) { (e.currentTarget as HTMLDivElement).style.transform = 'translateY(-4px)'; (e.currentTarget as HTMLDivElement).style.boxShadow = '0 12px 36px rgba(27,58,107,.1)' } }}
                onMouseLeave={e => { if (!isPro) { (e.currentTarget as HTMLDivElement).style.transform = 'scale(1)'; (e.currentTarget as HTMLDivElement).style.boxShadow = '0 4px 20px rgba(27,58,107,.05)' } }}
              >
                {isPro && (
                  <div style={{ position: 'absolute', top: -14, left: '50%', transform: 'translateX(-50%)', background: 'linear-gradient(135deg, #C9922A, #e8a83a)', color: '#fff', fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: '.7rem', padding: '5px 18px', borderRadius: 99, letterSpacing: '.06em', whiteSpace: 'nowrap', boxShadow: '0 4px 12px rgba(201,146,42,.4)' }}>
                    ✦ LE PLUS POPULAIRE
                  </div>
                )}
                {/* Plan name */}
                <div style={{ marginBottom: 20 }}>
                  <div style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: '1.05rem', color: isPro ? '#fff' : 'var(--text)', marginBottom: 6 }}>{plan.name}</div>
                  <div style={{ fontSize: '.82rem', color: isPro ? 'rgba(255,255,255,.6)' : 'var(--muted)', lineHeight: 1.5 }}>{plan.desc}</div>
                </div>
                {/* Price */}
                <div style={{ marginBottom: 28 }}>
                  {plan.priceMonthly === 0 ? (
                    <div style={{ fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: '1.8rem', color: isPro ? '#fff' : plan.color, letterSpacing: '-.04em', lineHeight: 1 }}>{plan.currency}</div>
                  ) : (
                    <div style={{ display: 'flex', alignItems: 'baseline', gap: 4 }}>
                      <span style={{ fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: '2.4rem', color: isPro ? '#C9922A' : plan.color, letterSpacing: '-.05em', lineHeight: 1 }}>{price}</span>
                      <span style={{ fontSize: '.9rem', color: isPro ? 'rgba(255,255,255,.5)' : '#94a3b8', fontWeight: 600 }}>DH/mois</span>
                    </div>
                  )}
                  {annual && plan.priceMonthly > 0 && (
                    <div style={{ fontSize: '.72rem', color: isPro ? 'rgba(255,255,255,.45)' : '#94a3b8', marginTop: 6 }}>
                      Facturé {price * 12} DH/an · Économisez {(plan.priceMonthly - plan.priceAnnual) * 12} DH
                    </div>
                  )}
                </div>
                {/* Divider */}
                <div style={{ height: 1, background: isPro ? 'rgba(255,255,255,.1)' : 'rgba(27,58,107,.07)', marginBottom: 24 }} />
                {/* Features */}
                <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 28 }}>
                  {plan.features.map((f, fi) => (
                    <li key={fi} style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                      <div style={{ width: 18, height: 18, borderRadius: '50%', background: isPro ? 'rgba(201,146,42,.2)' : `${plan.color}14`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, marginTop: 1 }}>
                        <svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke={isPro ? '#C9922A' : plan.color} strokeWidth="3" strokeLinecap="round"><polyline points="20 6 9 17 4 12"/></svg>
                      </div>
                      <span style={{ fontSize: '.83rem', color: isPro ? 'rgba(255,255,255,.82)' : '#374151', lineHeight: 1.5 }}>{f}</span>
                    </li>
                  ))}
                  {plan.disabled && plan.disabled.map((f, fi) => (
                    <li key={`d${fi}`} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, opacity: .4 }}>
                      ...
                      <span style={{ fontSize: '.83rem', color: '#94a3b8', lineHeight: 1.5, textDecoration: 'line-through' }}>{f}</span>
                    </li>
                  ))}
                </ul>
                {/* CTA */}
                <button style={{
                  width: '100%', padding: '13px', borderRadius: 11, border: isPro ? 'none' : `1.5px solid ${plan.color}25`,
                  background: isPro ? 'linear-gradient(135deg, #C9922A, #e8a83a)' : plan.priceMonthly === 0 ? plan.color : '#fff',
                  color: isPro ? '#fff' : plan.priceMonthly === 0 ? '#fff' : plan.color,
                  fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: '.88rem', cursor: 'pointer',
                ...
                >
                  {plan.priceMonthly === 0 ? 'Nous contacter' : 'Démarrer maintenant'}
                </button>
              </div>
            )
          })}
        ...
        <div style={{ textAlign: 'center', marginTop: 44 }}>
          <p style={{ fontSize: '.82rem', color: '#94a3b8' }}>Tous les plans incluent un essai gratuit de 14 jours · Aucune carte bancaire requise · Annulation à tout moment</p>
        </div>
      </div>
    </section>
  )
...
export default function HomePage() {
  ...
  const openCandidature = () => setShowModal(true)
  const closeCandidature = () => setShowModal(false)

  return (
    <div>
      <Navbar scrolled={scrolled} onOpenCandidature={openCandidature} />
      <main>
        <Hero onOpenCandidature={openCandidature} />
        <FeatureStrip />
        <ImagineSection />
        <PlatformDesc />
        <NotificationSection />
        ...
        <SocialProof />
        <TestimonialSection />
        <CTASection onOpenCandidature={openCandidature} />
      </main>
      <Footer />

      {showModal && <CandidatureModal onClose={closeCandidature} />}
    </div>
  )
}
